...
export const StreamProvider = ({ children }) => {
  ...
  const getNetworks = async () => {
    const networks = await StreamAPI.getNetworks()
      .then((res) => {
        return res.data;
      })
      .catch((err) => {
  ...
  const getSources = async () => {
    ...
    StreamAPI.getSources()
      .then((res) => {
        const sources = res.data
          .map((source) => {
            const match = networks.find(
              (network) =>
                source.name.replace(' ', '') === network.name.replace(' ', '')
            );
            return {
              ...source,
              id: match?.id,
  ...
  return (
    <StreamContext.Provider
      value={{ genres, sources, getTitles, getTitle, getTitleDetails }}
    >
      {children}
    </StreamContext.Provider>
  );
